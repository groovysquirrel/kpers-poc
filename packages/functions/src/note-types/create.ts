...
import { NoteTypeCreateInput, NoteTypeRow } from "../shared/types";
import * as uuid from "uuid";

export const main = handler(async (event) => {
  let data: NoteTypeCreateInput;
  ...
  const id = uuid.v4();
  console.log("[NoteTypes] Create - Starting", { id });
  ...
  console.log("[NoteTypes] Create - Note type created", { id });
