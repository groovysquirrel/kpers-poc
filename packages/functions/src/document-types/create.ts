...
import { DocumentTypeCreateInput, DocumentTypeRow } from "../shared/types";
import * as uuid from "uuid";

export const main = handler(async (event) => {
  let data: DocumentTypeCreateInput;
  ...
  const id = uuid.v4();
  console.log("[DocumentTypes] Create - Starting", { id });
  ...
  // Fetch the created document type
  const selectSql = `
  ...
  console.log("[DocumentTypes] Create - Document type created", { id });
